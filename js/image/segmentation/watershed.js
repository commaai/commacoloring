 ...
 *
 */
define(["./base", "./binary-heap-priority-queue", "../canny", "../compat", "../distance-transform"], function (BaseSegmentation, PriorityQueue, canny, compat, distanceTransform) {
  ...
  function WatershedSegmentation(imageData, options) {
    ...
    options = options || {};
    this.sigmaRange = options.sigmaRange || [-2, -1, 0, 0.5, 1, 2, 3].map(function (n) {
      return Math.pow(2, n);
    });
    ...
    this.neighborMap4 = new NeighborMap(this.imageData.width, this.imageData.height, [[-1, -1], [-1, 0], [-1, 1], [0, -1]]);
    this._compute();
  }
  ...
  WatershedSegmentation.prototype._findLocalMaxima = function (intensity) {
    ...
    for (offset = 0; offset < suppressed.length; ++offset) {
      if (suppressed[offset]) offsets.push(offset);
    }return offsets;
  };

  WatershedSegmentation.prototype._findUniqueRegionLabel = function (neighbors, labels) {
    var uniqueLabels = [];
  ...
  WatershedSegmentation.prototype._findDominantLabel = function (neighbors, labels, target) {
    var histogram = {},
        label;
    for (var i = 0; i < neighbors.length; ++i) {
      label = labels[neighbors[i]];
      if (label !== target) {
        if (histogram[label]) ++histogram[label];else histogram[label] = 1;
      }
    }
    var count = 0,
        dominantLabel = null;
  ...
  WatershedSegmentation.prototype.erode = function (target, labels) {
    ...
    for (offset = 0; offset < labels.length; ++offset) {
      if (labels[offset] === target) offsets.push(offset);
    ...
    while (offsets.length > 0) {
      ...
      if (dominantLabel !== null) {
        ...
        if (updates[dominantLabel]) ++updates[dominantLabel];else updates[dominantLabel] = 1;
        --updates[target];
  ...
  // Find small item.
  WatershedSegmentation.prototype._findSmallLabel = function (histogram) {
    var smallLabel = null;
  ...
  WatershedSegmentation.prototype._removeSmallRegions = function (labels) {
    var histogram = {},
        offset,
        label,
        updates;
